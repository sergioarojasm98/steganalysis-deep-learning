...
def plot_classification_density(y_true, y_pred_prob):
    ...
    # Establece las leyendas manualmente para garantizar que se muestren
    handles, labels = plt.gca().get_legend_handles_labels()
    plt.legend(handles=handles, labels=labels, title="Clase")
...
def plot_classification_density_by_method(y_true, y_pred_prob, method):
    ...
    sns.kdeplot(
        ...
        palette=["red", "blue"],
        alpha=0.7,
        ax=ax,  # especificamos el objeto ax
    ...
    # Mover la leyenda fuera de la gráfica para que no se sobreponga
    ax.legend(
...
def calculate_correlation_scipy(errors, num_chars_list):
    correlation, _ = pearsonr(errors, num_chars_list)
    return correlation
