...
def save_history_as_csv(history):
    ...
    send_telegram_message(
        f"[CRATOS] Tu historial de entrenamiento se guardó en {file_path}"
    )
...
def save_roc_curve_data_as_csv(y_true, y_pred_prob):
    ...
    df.to_csv(file_path, index=False)
    send_telegram_message(
        f"[CRATOS] Los datos de tu curva ROC se guardaron en {file_path}"
    )


def save_confusion_matrix_as_csv(cm):
    script_name = os.path.basename(__file__).split(".")[0]
    file_path = os.path.join(
        "/home/srojas/tg2/Resultados/CSV_Files/", f"Confusion_Matrix_{script_name}.csv"
    )
...
def save_frr_far_data_as_csv(y_true, y_pred_prob):
    ...
    df.to_csv(file_path, index=False)
    send_telegram_message(
        f"[CRATOS] Los datos de FAR y FRR se guardaron en {file_path}"
    )
...
def plot_far_frr(y_true, y_pred_prob):
    ...
    send_telegram_message(f"[CRATOS] Tu gráfica de FAR vs FRR se guardó en {file_path}")
...
def plot_confusion_matrix(cm, classes):
    save_confusion_matrix_as_csv(cm)
    ...
    send_telegram_message(f"[CRATOS] La matriz de confusión se guardo en {file_path}")
...
def plot_classification_density(y_true, y_pred_prob):
    ...
    send_telegram_message(
        f"[CRATOS] La densidad del valor de clasificación se guardo en {file_path}"
    )
...
def plot_classification_distribution(y_true, y_pred_prob):
    ...
    send_telegram_message(
        f"[CRATOS] La distribucion del valor de clasificación se guardo en {file_path}"
    )


def test_best_model():
    script_name = "CNN_Test_7"
    ...
    message = "[CRATOS] Metricas con el conjunto de Test:\n"
...
def metrics_best_model():
    ...
    message = "[CRATOS] Otras metricas con el conjunto de Test:\n"
