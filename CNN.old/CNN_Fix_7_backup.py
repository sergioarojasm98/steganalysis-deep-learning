...
def save_confusion_matrix_as_csv(cm):
    script_name = os.path.basename(__file__).split(".")[0]
    file_path = os.path.join(
        "/home/srojas/tg2/Resultados/", f"Confusion_Matrix_{script_name}.csv"
    )
...
def save_roc_curve_data_as_csv(y_true, y_pred_prob):
    ...
    script_name = os.path.basename(__file__).split(".")[0]
    file_path = os.path.join(
        "/home/srojas/tg2/Resultados/", f"ROC_Data_{script_name}.csv"
    )
...
def test_best_model():
    script_name = "CNN_Test_7"  # os.path.splitext(os.path.basename(__file__))[0]
    ...
    mcc_value = calculate_mcc(cm)

    send_telegram_message(f"MCC: {mcc_value:.4f}")
