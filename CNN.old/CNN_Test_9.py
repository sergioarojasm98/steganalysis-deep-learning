...
def plot_confusion_matrix(cm, classes):
    plt.figure(figsize=(12, 8))  # Incrementa el tamaño de la figura
    ...
    plt.xticks(
        tick_marks, classes, rotation=45, ha="right"
    )  # Alinea los ticks a la derecha
    ...
    plt.tight_layout()
    plt.subplots_adjust(bottom=0.15)  # Ajusta el margen inferior
...
def plot_classification_density(y_true, y_pred_prob):
    ...
    plt.ylabel("Densidad")
    plt.title("Densidad del Valor de Clasificación")
    ...
    send_telegram_message(
        f"[CNN] La grafica de densidad del valor de clasificación se guardo en {file_path}"
    )
...
def plot_classification_distribution(y_true, y_pred_prob):
    ...
    send_telegram_message(
        f"[CNN] La distribucion del valor de clasificación se guardo en {file_path}"
    )
...
def plot_classification_density_by_method(y_true, y_pred_prob, method):
    ...
    plt.xlabel("Valor de Clasificación")
    plt.ylabel("Densidad")
    plt.title(f"Densidad del Valor de Clasificación ({method})")
    plt.legend(title="Clase")
    # Guardar CSV de los datos de la grafica
    file_path = os.path.join(
        "/home/srojas/tg2/Resultados/", f"Densidad_{method}_{SCRIPT_NAME}.csv"
    )
    df.to_csv(file_path, index=False)
    ...
    send_telegram_message(
        f"[CNN] La densidad del valor de clasificación para {method} se guardo en {file_path}"
    )
...
def plot_classification_distribution_by_method(y_true, y_pred_prob, method):
    ...
    file_path = os.path.join(
        "/home/srojas/tg2/Resultados/", f"Distribucion_{method}_{SCRIPT_NAME}.csv"
    )
    df.to_csv(file_path, index=False)
...
def plot_distribution(data, title):
    ...
    file_path = os.path.join(
        "/home/srojas/tg2/Resultados/", f"Distribution_{title}_{SCRIPT_NAME}.pdf"
    )
    plt.savefig(file_path)
    plt.close()


def extract_num_chars_from_path(path):
    methods = ["LSB", "DCT", "DWT"]
...
send_telegram_message(extract_num_chars_from_path(test_path))  # debería imprimir 12345
...
def calculate_errors(true_labels, predictions):
    # Error simple
    return np.abs(predictions - true_labels)
...
def calculate_correlation_scipy(errors, num_chars_list):
    correlation, _ = pearsonr(predicted_errors, num_chars_list)
    return correlation
...
def test_and_metrics_by_method():
    ...
    for method, data in divided_data.items():
        if method != "ORIGINAL":
            ...
            mcc_value = compute_mcc(cm)
            eer, threshold_eer = plot_far_frr(combined_labels, predictions)
            ...
            predicted_errors = calculate_errors(combined_labels, predictions)

            # Extraer el número de caracteres de cada imagen
            num_chars_list = [
                extract_num_chars_from_path(path) for path in combined_paths
            ]

            send_telegram_message(
                predictions[:10]
            )  # imprime las primeras 10 predicciones
            send_telegram_message(
                predicted_errors[:10]
            )  # imprime los primeros 10 errores
            send_telegram_message(
                num_chars_list[:10]
            )  # imprime los primeros 10 valores de num_chars_list

            plot_distribution(predicted_errors, "Predicted_Errors")
            plot_distribution(num_chars_list, "Hidden_Characters")

            # También puedes guardar estos valores en un archivo para una inspección más detallada
            with open("log_errors.txt", "w") as file:
                for error in predicted_errors:
                    file.write(f"{error}\n")

            with open("log_num_chars.txt", "w") as file:
                for num_chars in num_chars_list:
                    file.write(f"{num_chars}\n")

            # Calcular la correlación
            correlation_numpy = calculate_correlation_numpy(
                predicted_errors, num_chars_list
            )
            correlation_scipy = calculate_correlation_scipy(
                predicted_errors, num_chars_list
            )

            message = f"[CNN] Correlación (numpy) entre el error y el número de caracteres ocultos para {method}: {correlation_numpy:.4f}\n"
            message += f"[CNN] Correlación (scipy) entre el error y el número de caracteres ocultos para {method}: {correlation_scipy:.4f}\n"
            send_telegram_message(message)
...
def main():
    ...
    send_telegram_message(
        "[CNN] Tu programa {} ha terminado de ejecutarse.".format(
