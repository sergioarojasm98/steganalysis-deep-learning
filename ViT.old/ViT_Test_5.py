...
for folder_path, label in zip(folder_paths, labels):
    for image_file in os.listdir(folder_path):
        ...
        try:
            ...
            send_telegram_message(
                f"[ViT] Error al abrir la imagen: {full_path}. Error: {e}"
            )
    send_telegram_message(f"[ViT] Carpeta {folder_path} procesada.")
...
train_loader = DataLoader(
    train_dataset, batch_size=96, sampler=sampler, num_workers=12, pin_memory=True
)
validation_loader = DataLoader(
    validation_dataset, batch_size=96, shuffle=False, num_workers=12, pin_memory=True
)

test_dataset = CustomDataset(test_paths, test_labels, transform=transform)
test_loader = DataLoader(
    test_dataset, batch_size=96, shuffle=False, num_workers=12, pin_memory=True
)
...
def train_and_validate(epochs):
    ...
    for epoch in range(1, epochs + 1):
        ...
        if avg_loss < best_val_loss:
            ...
            send_telegram_message(
                f"Guardado el mejor modelo con pérdida de validación: {avg_loss:.6f} en {model_path}"
            )
    ...
    if best_val_metrics:
        ...
        send_telegram_message(
            "No se encontró una mejora en la pérdida de validación a lo largo de las épocas."
        )
