...
MODEL_PATH = os.path.join(MODELS_DIR, f"Best_Model_{SCRIPT_NAME}.h5")
...
if gpus:
    try:
        for gpu in gpus:
            # Habilitando el crecimiento de la memoria para cada GPU
...
def train_model():
    ...
    flat1 = Flatten()(drop1)  # Aplanar la salida para la concatenación posterior

    # Segunda capa convolucional
    conv2 = Conv2D(32, kernel_size=(3, 3), activation="relu")(drop1)
    ...
    flat4 = Flatten()(pool4)  # Aplanar la salida para la concatenación

    # Concatenar la salida aplanada de la primera y cuarta capa
    concat = concatenate([flat1, flat4])

    # Capas completamente conectadas
    dense1 = Dense(256, activation="relu")(concat)
    norm5 = BatchNormalization()(dense1)
    drop2 = Dropout(0.5)(norm5)
    ...
    best_model_name = os.path.join(MODELS_DIR, f"Best_Model_{SCRIPT_NAME}.h5")
...
def save_history_as_csv(history):
    file_path = os.path.join(
        CSV_DIR, f"Training_History_{SCRIPT_NAME}.csv"
    )
...
def save_roc_curve_data_as_csv(y_true, y_pred_prob):
    ...
    file_path = os.path.join(
        CSV_DIR, f"ROC_Data_{SCRIPT_NAME}.csv"
    )
...
def plot_far_frr_by_method(y_true, y_pred_prob, method):
    ...
    file_path = os.path.join(
        RESULTS_DIR, f"FAR_FRR_{method}_{SCRIPT_NAME}.pdf"
    )
    plt.savefig(file_path)
    plt.close()
...
def save_roc_curve_data_as_csv_by_method(y_true, y_pred_prob, method):
    ...
    file_path = os.path.join(
        CSV_DIR, f"ROC_Data_{method}_{SCRIPT_NAME}.csv"
    )
...
def plot_roc_curve_by_method(y_true, y_pred_prob, method):
    ...
    file_path = os.path.join(
        RESULTS_DIR, f"ROC_{method}_{SCRIPT_NAME}.pdf"
    )
    plt.savefig(file_path)
    plt.close()  # Cierre de la figura
...
def plot_confusion_matrix_by_method(cm, method, classes):
    ...
    file_path = os.path.join(
        RESULTS_DIR, f"Confusion_Matrix_{method}_{SCRIPT_NAME}.pdf"
    )
    plt.savefig(file_path)
    plt.close()
...
def plot_classification_density_by_method(y_true, y_pred_prob, method):
    ...
    # Guardar CSV de los datos de la gráfica
    file_path = os.path.join(
        CSV_DIR, f"Densidad_{method}_{SCRIPT_NAME}.csv"
    )
    data.to_csv(file_path, index=False)
    ...
    file_path = os.path.join(
        RESULTS_DIR, f"Densidad_{method}_{SCRIPT_NAME}.pdf"
    )
    plt.savefig(file_path)
    plt.close()
...
def plot_classification_distribution_by_method(y_true, y_pred_prob, method):
    ...
    file_path = os.path.join(
        RESULTS_DIR, f"Distribucion_{method}_{SCRIPT_NAME}.pdf"
    )
    plt.savefig(file_path)
    plt.close()
